...
then
	...
	python -u tools/infer_simple.py --dataset keypoints_coco2017 --cfg ./configs/baselines/e2e_keypoint_rcnn_R-50-FPN_1x.yaml --load_detectron ./data/weights/R-50-FPN-1x.pkl --image_dir ../../dataset/all/processed/${filename}.MP4 --output_dir ../../dataset/all/annotations/${filename} # next time add --annotations and --video
  cd ..
fi
...
then
	...
	python ./scripts/hungarian.py --filename ../dataset/all/annotations/${filename}.txt --maximum_allowed_distance ${15} --maximum_limbo_lifetime ${16}
	python ./scripts/classify_trajectories.py --filename ../dataset/all/annotations/${filename}-modified.txt --tl ${11} ${12} --tr ${13} ${14} --br ${15} ${16} --bl ${17} ${18}
fi

if [ "$annotate_video" -eq 1 ]
then
	echo Drawing trajectories video
	python ./scripts/draw_trajectories.py ../dataset/all/processed/${filename}.MP4 ../dataset/all/processed/${filename}-annotated.MP4 ../dataset/all/annotations/${filename}-modified.txt
fi
