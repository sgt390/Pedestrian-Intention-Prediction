...
op_cap = cv2.VideoWriter(op_video, cv2.VideoWriter_fourcc('H', '2', '6', '4'), 30,
                         (int(in_cap.get(3)), int(in_cap.get(4))))
...
while in_cap.isOpened():
    ...
    for row in df[df["frame"] == t].itertuples(index=True, name='Pandas'):
        ...
        if row.cross == 1 and row.lifetime > min_lifetime_crossers:
            cv2.putText(im, "crossing", (row.tlx, row.tly + row.height + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 0, 255), 1)
            cv2.putText(im, str(row.id), (row.tlx, row.tly + row.height - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (0, 0, 255), 2)
            cv2.rectangle(im, (row.tlx, row.tly), (row.tlx + row.width, row.tly + row.height), color=(0, 0, 255),
                          thickness=2)

        if row.cross == 0 and row.lifetime > min_lifetime_noncrossers:
            cv2.putText(im, str(row.id), (row.tlx, row.tly + row.height - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        (255, 0, 0), 2)
            cv2.rectangle(im, (row.tlx, row.tly), (row.tlx + row.width, row.tly + row.height), color=(255, 0, 0),
                          thickness=2)
