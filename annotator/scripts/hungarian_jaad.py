...
parser.add_argument('--maximum_allowed_distance', type=int)
parser.add_argument('--maximum_limbo_lifetime', type=int)
#
args = parser.parse_args()
filename = args.filename  # "./annotations/Ouchy-1-Right-Short.txt"
max_distance = args.maximum_allowed_distance
max_lifetime = args.maximum_limbo_lifetime
...
def hungarian(df, max_distance, max_lifetime, save=False):
        ...
        if np.shape(x2)[0] != 0:
            ...
            if np.shape(x1)[0] != 0:
                ...
                for r, c in zip(row_index_priority, col_index_priority):
                    ...
                    if ((distances_priority[r][c] < max_distance) and (
                            np.float(np.minimum(h1[r], h2[c])) / np.float(np.maximum(h1[r], h2[c])) > 0.5)):
                        Y[-1][c] = Y[-2][r]
            ...
            if np.shape(x1_buf)[0] != 0:
                ...
                for r, c in zip(row_index, col_index):
                    ...
                    # then remove them
                    if r >= np.shape(x1)[0]:
                        if ((distances[r][c] < max_distance) and (
                                np.float(np.minimum(h1_buf[r], h2[c])) / np.float(np.maximum(h1_buf[r], h2[c])) > 0.5)):
    ...
    # rewrite ground truth with label (change IDs to integers)
    print("\nAssigning labels")
...
if __name__ == '__main__':
    ...
    hungarian(df, max_distance, max_lifetime, save=True)
