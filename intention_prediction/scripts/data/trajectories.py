...
def JAADcollate(data):
    # calls the __getitem__ method
    (pedestrian_images, standing, looking, walking, crossing, pedestrian_foldernames, pedestrian_filenames,scenes_images, scenes_foldernames, scenes_filenames) = zip(*data)

    # cannot convert to tensor due to variable length
    standing = torch.tensor(standing, dtype=torch.long)
    looking = torch.tensor(looking, dtype=torch.long)
    walking = torch.tensor(walking, dtype=torch.long)
    crossing = torch.tensor(crossing, dtype=torch.long)

    return [pedestrian_images, standing, looking, walking, crossing, pedestrian_foldernames, pedestrian_filenames, scenes_images, scenes_foldernames, scenes_filenames]
...
def LausanneDataset(data_dir, min_obs_len=8, max_obs_len=8, timestep=10):
    ...
    for i in df["unique_id"].unique():
        ...
        while curr_frame <= df_temp["frame"].max():
            try:
                ind_temp.append(np.min(df_temp[(df_temp["frame"] >= curr_frame) & (
                            df_temp["frame"] < curr_frame + timestep)].index.tolist()))
            except ValueError:
                pass
            curr_frame += timestep
...
def JAADDataset(data_dir, min_obs_len=10, max_obs_len=10, timestep=1):
    ...
    for i in df["unique_id"].unique():
        ...
        if ((len(np.where(df_temp['incrossing'] > 0)[0]) == 0) and (
                len(df_temp[df_temp['incrossing'] == 1]) == 0)):  # it was incrossing
    ...
    # --------------------------------------------------------------
    # get pedestrian frames in steps of 'timestep'
    # better than older implementation since it takes the max,making it
    # impossible to miss out the final crossing decision
    ...
    for i in df["unique_id"].unique():
        ...
        while curr_frame >= df_temp["frame"].min():
            try:
                ind_temp.append(np.max(df_temp[(df_temp["frame"] <= curr_frame) & (
                            df_temp["frame"] > curr_frame - timestep)].index.tolist()))
            except ValueError:
                pass
            curr_frame -= timestep
...
class LausanneLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return [pedestrian_images, df["incrossing"][-1], pedestrian_folderpaths, pedestrian_filenames]
    # -------------------------------
